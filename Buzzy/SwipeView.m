...
static CGFloat const  kBackgroundCardsTopMargin                   = 0.0;
static CGFloat const  kBackgroundCardsScalePercent                = 1.0;
static CGFloat const  kBackgroundCardsLeftMargin                  = 0.0;
...
static NSTimeInterval const kAppearAlphaAnimationDuration         = 0.7;

//Opacity values
static CGFloat const kDefaultAlphaValueOpaque                     = 1.0;
static CGFloat const kDefaultAlphaValueTransparent                = 0.0;
static CGFloat const kDefaultAlphaValueSemiTransparent            = 1.0;
...
- (void)swipeToDirection:(SwipeDirection)direction
{
    ...
    DraggableCardView *oldCard = self.visibleCardsViewArray[0];
    [oldCard.contentView.videoPlayer stop];
    oldCard.contentView.dead = YES;
    [oldCard.contentView.videoPlayer setMuted:YES];
    oldCard.contentView.videoPlayer.delegate =nil;
    oldCard.contentView.videoPlayer = nil;
...
- (void)cardViewTapped:(DraggableCardView *)cardView position:(NSInteger)position totalUsers:(NSInteger)totalUsers{
    ...
    NSUInteger index = [self.visibleCardsViewArray indexOfObjectIdenticalTo:cardView];
    
    
    
    if ([self.delegate respondsToSelector:@selector(swipeView:didSelectCardAtIndex:position:totalUsers:)]) {
        [self.delegate swipeView:self didSelectCardAtIndex:index position:position totalUsers:totalUsers];
    }
}
